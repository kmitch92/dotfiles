...
**Configuration**: `tmux/.tmux.conf` (Plugins section)

### Session Save / Restore (tmsave / tmload)

Session persistence is handled by two shell commands, not by a tmux plugin:
`tmsave` writes the current session to JSON under `~/.local/share/tmsave/`,
`tmload` restores a saved session into a new tmux session. Sources live in
`config/.config/tmux/scripts/`. These replaced tmux-resurrect and
tmux-continuum, which were configured but never actually installed.
