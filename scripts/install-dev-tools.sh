...
# =============================================================================
# Installs neovim, tmux, starship, and other essential dev tools
...
check_nvim_version() {
    ...
    local current=$(echo "$version_string" | grep -oP 'v?\K[0-9]+\.[0-9]+\.[0-9]+' | head -n1)
...
install_neovim_appimage() {
    ...
    print_info "Detected architecture: $arch (using $appimage_name)"
    ...
    if curl -LO "$download_url"; then
        ...
        if [ -f "$appimage_name" ] && [ $(stat -c%s "$appimage_name") -gt 1000000 ]; then
